...
PROJECT_DIR="${1:?usage: build-docs-examples.sh <docusaurus-project-dir> <out-examples-dir>}"
OUT_DIR="${2:?usage: build-docs-examples.sh <docusaurus-project-dir> <out-examples-dir>}"
...
"$SCRIPT_DIR/gqlmd.sh" --options "--homepage data/anilist.md --schema https://graphql.anilist.co/ --base . --link /examples/default --force --pretty --deprecated group"
...
"$SCRIPT_DIR/gqlmd.sh" --id schema_with_grouping --options "--homepage data/groups.md --schema data/schema_with_grouping.graphql --groupByDirective @doc(category|=Common) --base . --link /examples/group-by --skip @noDoc --index --noParentType --deprecated group --hierarchy entity"
