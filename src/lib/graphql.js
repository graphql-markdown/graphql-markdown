...
function getTypeName(type, name = '') {
    if (type) return ('name' in type && type.name) || ('toString' in type && type.toString()) || name;
    return undefined;
}
...
function getSchemaMap(schema) {
    ...
    return {
        queries: getIntrospectionFieldsList(schema.getQueryType && schema.getQueryType()),
        mutations: getIntrospectionFieldsList(schema.getMutationType && schema.getMutationType()),
        subscriptions: getIntrospectionFieldsList(schema.getSubscriptionType && schema.getSubscriptionType()),
