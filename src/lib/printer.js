...
module.exports = class Printer {
    ...
    printCodeArguments(type) {
        ...
        if ('args' in type && type.args.length > 0) {
            ...
            code += type.args.reduce((r, v) => {
                ...
                return `${r}  ${v.name}: ${v.type.toString()}${defaultValue ? ` = ${defaultValue}` : ''}\n`;
            }, '');
            code += `)`;
        }
        return code;
    ...
    printCodeType(type) {
        let code = `${isInterfaceType(type) ? 'interface' : 'type'} ${getTypeName(type)}`;
        code += `${
        ...
        if (type) {
            ...
            if (isObjectType(type) || isInterfaceType(type) || isInputObjectType(type)) {
                ...
                if ('getInterfaces' in type)
                    metadata += this.printSection(type.getInterfaces && type.getInterfaces(), 'Interfaces');
            }
            ...
            return prettifyMarkdown(`${header}\n\n${description}\n\n${code}\n\n${metadata}\n\n`);
        }
        return '';
    }
};
