    ...
    return {
        ...
        extendCli(cli) {
            cli.command('graphql-to-doc')
                ...
                .option("-r, --root <rootPath>", "Root folder for doc generation", config.rootPath)
                .option("-b, --base <baseURL>", "Base URL to be used by Docusaurus", config.baseURL)
                .option("-l, --link <linkRoot>", "Root for links in documentation", config.linkRoot)
                .option("-h, --homepage <homepage>", "File location for doc landing page", config.homepage)
                .option("-f, --force", "Force document generation")
                .option("-d, --diff <diffMethod>", "Set diff method", config.diffMethod)
                .option("-t, --tmp <tmpDir>", "Set temp dir for schema diff", config.tmpDir)
                .description('Generate GraphQL Schema Documentation')
                .action(async (options) => {
                    baseURL = options.base;
                    linkRoot = options.link;
                    schema = options.schema;
                    rootPath = path.join(options.root, baseURL);
                    homepage = options.homepage;
                    diffMethod = options.force ? "FORCE" : options.diff;
                    tmpDir = options.tmp;
                    await generateDocFromSchema(baseURL, schema, rootPath,  linkRoot, homepage, diffMethod, tmpDir);
                });
        },
    };
};
