...
export class Printer implements IPrinter {
  ...
  /**
   * Prints meta tags for the documentation using Docusaurus Head component.
   *
   ...
   *
   * The generator tag is suppressed when either `suppressGenerator` is `true`
   * or the user has already provided a `name="generator"` entry in `metatags`,
   * preventing duplicate generator tags.
   *
   * Output is wrapped in Docusaurus `<Head>` component for proper meta tag
   * injection into the document head.
   *
   * @param _type - GraphQL type (unused)
   * @param options - Printer configuration options containing metatags
   * @returns Formatted meta tags wrapped in Docusaurus Head component
   */
