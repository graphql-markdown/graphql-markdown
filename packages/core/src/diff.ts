...
export interface CheckSchemaChanges {
  (schema: GraphQLSchema, tmpDir: string, diffMethod?: unknown): Promise<boolean>
}
...
export const hasChanges = async (
  ...
  try {
    const { checkSchemaChanges }: {checkSchemaChanges: CheckSchemaChanges} = await import(diffModule);
